	...
	public static void main(String[] args) {
	    ...
	    while(true) {
	    	...
	    	if(ch==1) {
	    		...
	    		if( 환타바구니 >0 ) { System.out.println("콜라  "+ 환타바구니 + 환타가격* 환타바구니);  total +=환타가격*환타바구니 ; }
	    		if( 사이다바구니 >0 ) { System.out.println("콜라  "+ 사이다바구니 + 사이다가격* 사이다바구니);  total +=사이다가격*사이다바구니 ; }
	    		
	    		System.out.println("총결제금액 : " + total);
	    		System.out.println("1: 결제 2: 취소"); int pay = in.nextInt();
	    		if(pay ==1 ) {
	    			System.out.println("금액을 입력하세요"); int  price = in.nextInt();
	    			if( price >= total ) {
