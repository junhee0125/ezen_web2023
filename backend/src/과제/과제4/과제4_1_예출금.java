	...
	public static void main(String[] args) { 
		...
		while( true ) { // 무한루프 [ 종료조건 : 4 입력시 break; ] 
			...
			if(ch == 1) { // 예금기능3
				...
				if(scanner.next().equals(account)) { // 계좌번호 입력받아 계좌번호 일치하면
				
					System.out.print("예금액을 입력하세요 : "); //예금액 입력받는다.
					int in = scanner.nextInt(); //in 변수에 받아서
					balance += in;	// 잔고에 더해준다.
					System.out.println("예금후 금액" + balance); //예금성공 메시지 출력한다.
				} else { //계좌번호 일치하지 않으면 
					System.out.println("없는 계좌번호입니다."); // 계좌번호 불일치 메시지 출력
				}
				...
				if(scanner.next().equals(account)) {
					...
					if(scanner.nextShort() == password) { //비밀번호가 일치하면 
						...
						if(balance < out) {	System.out.println("잔액부족");	}  //잔고보다 춤금액이 크면			
						else {  balance -= out; System.out.println("출금후 금액" + balance);	}
						
					}  else {
						System.out.print("해당 계좌의 비밀번호가 다릅니다. ");
					}
				} else {
					System.out.println("없는 계좌번호 입니다..");
				}
				
			} else if (ch == 3 ) {  //3. 잔고 출력
				
				System.out.print("예금잔액: " + balance);
				
			} else if (ch == 4) { //4. while문 종료
