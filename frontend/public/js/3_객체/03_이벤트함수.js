...
// 1. document 객체 이용한 태그을 객체로 호출 
	...
	document.body.style = 'background-color:blue; font-size:20px; color:white;'
	...
	/*
		이벤트명 
			DOMContentLoaded : HTML이 완전히 열렸을때[모두 로드되었을 때] 실행되는 이벤트
			window.onload : 해당객체가 호출되었을때 실행되는 이벤트  // div가능
	...
	//2. 해당객체가 호출되었을 때
	window.onload = ()=>{
		console.log('JS에서 이벤트 실행2');
	};
	
	//3. JS가 열렸을 때 이벤트 1번 실행  (여기까지 실행순서  3 > 1 > 2)
	
	console.log('JS에서 이벤트 실행3'); // 
	
	//4. 
	let p = document.querySelector('p'); console.log(p); // (여기까지. 4 > 3 > 1 > 2)
